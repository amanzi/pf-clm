...
clm_last_rst,clm_daily_rst)
  ...
  character (LEN=clm_output_dir_length) :: clm_output_dir ! output dir locatio
n
  ! ET keys
...
!       writing of the log files. This greatly reduces the number of files created during a run.
  ...
  if (clm_write_logs==1) write(999,*) "clm.F90: rank =", rank, "   istep =", istep_pf
                                                                                                                                                        ! clm_create
  !=== Specify grid size using values passed from PF                                                                                                    ! set up drv, address timestep questions?
  drv%dx = pdx
  drv%dy = pdy
  drv%dz = pdz
  drv%nc = nx
...
!
  ...
  write(9919,*) "CLM starting istep =", istep_pf
     ...
     !=== Allocate Memory for Grid Module                                                                                                               ! grid_create
     ...
     call drv_readclmin (drv,grid,rank,clm_write_logs)                                                                                                  ! still must call, reads CLM input data, should move into a setter call
    ...
    !=== @RMM check for error in IC or starting time
     ...
     allocate (tile(drv%nch), stat=ierr); call drv_astp(ierr)                                                                                           ! call tile_create, clm1d_create
     allocate (clm (drv%nch), stat=ierr); call drv_astp(ierr)
...
!!!!! WE IS HERE
     ...
     call clm_typini(drv%nch,clm,istep_pf)                                                                                                              ! end of clm1d_create?
     ...
     do t = 1, drv%nch
        call drv_g2clm (drv%udef, drv, grid, tile(t), clm(t))                                                                                           ! need this for now, should move into a setup?
     enddo
     ...
     call drv_readvegpf (drv, grid, tile, clm)                                                                                                          ! need this for now, reads a clm file for veg params.  should move into a setter
...
!           enddo
 ...
 !!          ! PRINT CHECK
     ...
     do t=1,drv%nch  
        ...
        if (clm(t)%planar_mask == 1) then
           ...
           clm(t)%threshold_type     = irr_thresholdtypepf
                                                                                                                                                        ! call host_to_clm_wc, host_to_clm_tk
           ! set clm watsat, tksatu from PF porosit
     ...
     call drv_restart(1,drv,tile,clm,rank,istep_pf)        ! (1=read,2=write)                                                                           ! need this, loads restart file
  ...
  call pfreadout(clm,drv,tile,saturation,pressure,rank,ix,iy,nx,ny,nz,j_incr,k_incr,ip)                                                                 ! call host_to_clm_pressure,host_to_clm_wc

  !=== Advance time (CLM calendar time keeping routine)
  drv%endtime = 0
  call drv_tick(drv)                                                                                                                                    ! need this, related to timestep
...
!
  ...
  call drv_getforce(drv,tile,clm,nx,ny,sw_pf,lw_pf,prcp_pf,tas_pf,u_pf,v_pf, &                                                                          ! call host_to_clm_met
  ...
  if (clm_1d_out == 1) then 
     call drv_1dout (drv, tile,clm,clm_write_logs)                                                                                                      ! need this, output
  endif


  !=== Call 2D output routine                                                                                                                           ! need this, output
  ...
  !=== Copy values from 2D CLM arrays to PF arrays for printing from PF (as Silo)                                                                       ! call clm_to_host_energy_flux, mass_flux, diagnostics, ??
  do t=1,drv%nch
     i=tile(t)%col
     j=tile(t)%row
...
! if ( (drv%gmt==0.0).or.(drv%endtime==1) ) call drv_restart(2,drv,tile,clm,rank,istep_pf)
  ...
  call pf_couple(drv,clm,tile,evap_trans,saturation,pressure,porosity,nx,ny,nz,j_incr,k_incr,ip,d_stp)                                                          ! call clm_to_host_total_mass_fluxes
                                                                                                                                                                !, but also includes some mass balance code that needs to go somewhere
  ...
  if (istep_pf==1) call drv_pout(drv,tile,clm,rank)                                                                                                             ! need this
  endif

  !=== If at end of simulation, close all files                                                                                                                 ! need to add call to close i/o
