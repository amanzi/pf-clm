...
subroutine clm1d_leaftem (z0mv,       z0hv,       z0qv,           &
                        thm,        th,         thv,            & 
                        tg,         qg,         dqgdT,          &
                        htvp,       sfacx,      dqgmax,         &
                        emv,        emg,        dlrad,          &
...
!=========================================================================
! $Id: clm1d_leaftem.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
!=========================================================================
...
! Declare Modules and data structures
  ...
  use clm1d_varcon, only : sb, cpair, hvap, vkc, grav
  implicit none
...
! Saturated vapor pressure and humidity and their derivation
  ...
  call clm1d_qsadv (clm%t_veg, clm%forc_pbot, el, deldT, qsatl, qsatldT)
...
! Initialize flux profile
  ...
  if (zldis < 0.0d0) zldis = 5.0d0  !@ sk Must be consistent with value in clm1d_obult.F90

  call clm1d_obuini(ur,thv,dthv,zldis,z0mv,um,obu)
...
! Evaluate stability-dependent variables using moz from prior iteration
     ...
     call clm1d_obult (clm%displa, z0mv, z0hv, z0qv, obu, um, ustar, temp1, temp2, clm)
...
!  @BR "alpha" in weighting set to 2, csoilc changed in clm1d_input.dat to 0.0025
! @CLM Dry Bias
...
! should do each iteration to account for differences in eah, tv.
     ...
     call clm1d_stomata(mpe      , clm%parsun, svpts     , eah       ,    &
                      thm      , o2        , co2       ,                &
     ...
     call clm1d_stomata(mpe      , clm%parsha, svpts     , eah       ,    &
                      thm      , o2        , co2       ,                &
...
! Heat conductance for air, leaf and ground  
     ...
     call clm1d_condch(rah(1),rb,rah(2),wta,wtl,wtg,wta0,wtl0,wtg0, &
          wtal,wtga,wtgl,clm)
...
! Air has same conductance for both sensible and latent heat.
     ...
     call clm1d_condcq(raw(1),rb,raw(2),rpp,wtaq,wtlq,wtgq,wtaq0, &
