...
subroutine clm1d_main (clm,day,gmt)
  ...
  !   interface begins with clm1d_main.f90 and whose interaction with
  !   the driver occurs through the clm1d_module.  An example, highly flexible 
  ...
  !     clm1d_???.f90 = core CLM 1-D subroutines
  ...
  !
  ! clm1d_main FLOW DIAGRAM
  !    -> clm1d_dynvegpar:            ecosystem dynamics: phenology, vegetation, soil carbon 
  !    -> clm1d_coszen:               cosine solar zenith angle for next time step
  !    -> clm1d_surfalb:              albedos for next time step 
  !       -> clm1d_snowalb:           snow albedos: direct beam
  !       -> clm1d_snowalb:           snow albedos: diffuse
  !       -> clm1d_soilalb:           soil/lake albedos
  !       -> clm1d_twostream:         absorbed, reflected, transmitted solar fluxes (vis dir)   
  !       -> clm1d_twostream:         absorbed, reflected, transmitted solar fluxes (vis dif)  
  !       -> clm1d_twostream:         absorbed, reflected, transmitted solar fluxes (nir dir)  
  !       -> clm1d_twostream:         absorbed, reflected, transmitted solar fluxes (nir dif)  
  !    -> clm1d_surfrad
  !    -> clm1d_hydro_canopy
  !    -> clm1d_thermal 
  !         bare soil =>   
  ...
  ! 
  ! CLM revision:
  !  The clm1d_* routines have undergone a major revision, where the CLM module is 
  ...
  !=========================================================================
  ! $Id: clm1d_main.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
  !=========================================================================
  ...
  ! ------------------- arguments -----------------------------------
  type (clm1d_type), intent(inout) :: clm    !CLM 1-D Module
  ...
  call clm1d_coszen (clm,day,coszen)

  call clm1d_surfalb (clm,coszen)

  ! -----------------------------------------------------------------
  ! Surface Radiation
  ! -----------------------------------------------------------------

  call clm1d_surfrad (clm)
  ...
  if (.not. clm%lakpoi) then   
     ...
     ! Irrigation is not applied *before* clm1d_hydro_canopy to allow intercpetion of spray irrigation
     ! clm1d_hydro_irrig determines whether to irrigate and how much
     ! ...spray and drip irrigation are then applied in clm1d_hydro_canopy by adding 
     ...
     call clm1d_hydro_irrig (clm,gmt)
     ...
     if (clm%snl < 0) then
        ...
        call clm1d_compact (clm)

        ! Combine thin snow elements

        call clm1d_combin (clm)

        ! Divide thick snow elements

        call clm1d_subdiv (clm)
     ...
     ! IMF @ 
     ! Irrigation now called above, before clm1d_hydro_canopy, to allow intercpetion of spray irrigation 
     ! if (clm%itypwat == istsoil) call clm1d_hydro_irrig (clm)
     ...
     ! updated from clm1d_combin) and ending water balance

     !     if (clm%itypwat==istwet .or. clm%itypwat==istice) call clm1d_hydro_wetice (clm)
  ...
  call clm1d_snowage (clm)

  ! -----------------------------------------------------------------
  ! Check the energy and water balance
  ! -----------------------------------------------------------------
  !
  !  call clm1d_balchk (clm, clm%istep)
  !@Stefan: End of change
  return
end subroutine clm1d_main
