...
subroutine clm1d_obult (displa,  z0m,      z0h,    z0q,    obu,  &
...
!=========================================================================
! $Id: clm1d_obult.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
!=========================================================================
...
!=== Local Variables =====================================================
  ...
  real(r8)  zldis,    & ! reference height "minus" zero displacement heght [m]
       clm1d_psi,       & ! stability function for unstable case
...
! Temperature profile
  ...
  if (zeta < -zetat) then           ! zeta < -1
     temp1=vkc/(log(-zetat*obu/z0h)-clm1d_psi(2,-zetat) &
          +clm1d_psi(2,z0h/obu) &
          +0.8*((zetat)**(-0.333)-(-zeta)**(-0.333)))
  else if (zeta < 0.) then         ! -1 <= zeta < 0
     temp1=vkc/(log(zldis/z0h)-clm1d_psi(2,zeta)+ &
          clm1d_psi(2,z0h/obu))
  else if (zeta <= 1.) then        !  0 <= ztea <= 1
...
! Humidity profile
  ...
  if (zeta < -zetat) then          ! zeta < -1
     temp2=vkc/(log(-zetat*obu/z0q)- &
          clm1d_psi(2,-zetat)+clm1d_psi(2,z0q/obu) &
          +0.8*((zetat)**(-0.333)-(-zeta)**(-0.333)))
  else if (zeta < 0.) then         ! -1 <= zeta < 0
     temp2=vkc/(log(zldis/z0q)- &
          clm1d_psi(2,zeta)+clm1d_psi(2,z0q/obu))
  else if (zeta <= 1.) then        !  0 <= ztea <= 1
