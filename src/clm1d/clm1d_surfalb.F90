subroutine clm1d_surfalb (clm, coszen)
...
! 
! ------------------------------------------------------------------------
! $Id: clm1d_surfalb.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
!-----------------------------------------------------------------------
...
! snow albedos: only if coszen > 0 and h2osno > 0
  ...
  if ( clm%h2osno > 0._r8 ) then
     ic=0; call clm1d_snowalb (clm, coszen, nband, ic, albsnd)
     ic=1; call clm1d_snowalb (clm, coszen, nband, ic, albsni)  
  else
     albsnd(:) = 0._r8
     albsni(:) = 0._r8
  endif
...
! ground surface albedos: only if coszen > 0
  ...
  call clm1d_soilalb (clm, coszen, nband, albsnd, albsni)      
...
! (ic=0) and diffuse flux (ic=1) only if coszen > 0
     ...
     do ib = 1, nband
        ic = 0
        call clm1d_twostream (clm     , ib , ic      , coszen  , vai     , &
