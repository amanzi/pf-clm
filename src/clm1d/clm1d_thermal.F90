...
subroutine clm1d_thermal (clm)
  ...
  !
  !  (3) Phase change (see clm1d_meltfreeze.F90)
  !
  !  FLOW DIAGRAM FOR clm1d_thermal.F90
  !
  !  thermal ===> clm1d_qsadv
  !               clm1d_obuini
  !               clm1d_obult
  ...
  !=========================================================================
  ! $Id: clm1d_thermal.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
  !=========================================================================

  use clm_precision
  use clm1d_type_module
  use clm1d_varcon, only : denh2o, denice, roverg, hvap, hsub, &
...
!print*,'temp-alpha1:',temp_alpha
  ...
  call clm1d_qsadv(tg,clm%forc_pbot,eg,degdT,qsatg,qsatgdT)
  ...
  if (clm%frac_veg_nosno == 0) then  
     ...
     call clm1d_obuini(ur,thv,dthv,zldis,z0mg,um,obu)

     ! Evaluated stability-dependent variables using moz from prior iteration

     niters=3
     do iter = 1, niters         ! begin stability iteration
        call clm1d_obult(0.0d0,z0mg,z0hg,z0qg,obu,um,ustar,temp1,temp2,clm)
        tstar = temp1*dth
        qstar = temp2*dqh
...
! or made a user input via PF
     ...
     call clm1d_leaftem(z0mv,z0hv,z0qv,thm,th,thv,tg,qg,dqgdT,htvp,sfacx,     &
  ...
  i = size(at)
  call clm1d_tridia (i ,at ,bt ,ct ,rt ,clm%t_soisno(clm%snl+1:nlevsoi))
  ...
  call clm1d_meltfreeze (fact(clm%snl+1), brr(clm%snl+1), hs, dhsdT, &
