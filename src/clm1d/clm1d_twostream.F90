...
subroutine clm1d_twoStream (clm, ib , ic , coszen, vai, &
     rho, tau, fab, fre   , ftd, &
     fti, gdir)
  ...
  ! 
  !-----------------------------------------------------------------------
  ! $Id: clm1d_twostream.F90,v 1.1.1.1 2006/02/14 23:05:52 kollet Exp $
  !-----------------------------------------------------------------------

  use clm_precision
  use clm1d_type_module
  use clm1d_varpar, only : numrad
  use clm1d_varcon, only : omegas, tfrz, betads, betais
  implicit none
  ...
  return
end subroutine clm1d_twostream
