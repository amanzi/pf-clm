...
!=========================================================================
  ...
  use clm1d_type_module             ! CLM tile variables 
  use clm1d_varcon, only : denice, denh2o, istwet, istice, istsoil, istslak, istdlak
...
! ========================================================================
  ...
  if (clm%itypwat == istdlak) then        !assume all lakes are deep lakes
     ...
     clm%dz(1:nlevlak) = drv%udef
     clm%z(1:nlevlak)  = drv%udef
     ...
     ! now set in host_to_clm_dz
     clm%dz(1:nlevsoi) = drv%udef
     clm%zi(0:nlevsoi) = drv%udef
     clm%z(1:nlevsoi) = drv%udef
...
! ========================================================================
  ...
  if (.not. clm%lakpoi) then  !not lake
     ...
     do i = -nlevsno+1, nlevlak
        clm%h2osoi_liq(i) = drv%udef
        clm%h2osoi_ice(i) = drv%udef
     end do
  endif
